...
<template>
  <Table class="min-w-max relative">
    <TableHeader class="sticky top-0 bg-white z-10">
      <TableRow>
        <TableHead class="w-[50px] sticky left-3 bg-white after:content-[''] after:block after:absolute after:-left-3 after:top-0 after:z-20 after:h-[calc(100%+2px)] after:w-3 after:bg-white">STT</TableHead>
        <TableHead class="sticky left-[calc(50px+0.75rem)] bg-white">Ma_PR</TableHead>
    ...
    <TableBody>
      <TableRow v-if="value.length === 0">
        <TableCell colspan="11" class="text-center">Không có yêu cầu nào</TableCell>
      </TableRow>
      <TableRow v-for="(row, index) in value" :key="row.Ma_PR">
        <router-link :to="`requests/${row.Ma_PR}`" class="contents">
          <TableCell class="sticky left-3 bg-white after:content-[''] after:block after:absolute after:-left-3 after:top-0 after:z-20 after:h-[calc(100%+2px)] after:w-3 after:bg-white">{{ index + 1 }}</TableCell>
          <TableCell class="sticky left-[calc(50px+0.75rem)] bg-white">{{ row.Ma_PR }}</TableCell>
          <TableCell>{{ formatISODate(row.Thoi_gian_yeu_cau) }}</TableCell>
          ...
          <TableCell>{{ formatISODate(row.Truong_BP_duyet) }}</TableCell>
          <TableCell>{{ formatISODate(row.Ke_toan_phe_duyet) }}</TableCell>
          <TableCell>{{ formatISODate(row.BLD_phe_duyet) }}</TableCell>
        </router-link>
        <TableCell class="sticky right-3 bg-white">
          <div v-if="canEdit(row.Trang_thai)" class="flex gap-3 items-center">
            <Pencil class="cursor-pointer" :size="20" @click="$emit('updateRequest', row)"/>
            <Dialog>
              <DialogTrigger>
              ...
              <DialogContent>
                ...
                <div>
                  <p>Bạn đang thực hiện xóa yêu cầu có id là <b>{{ row.Ma_PR}}</b>. Bạn muốn thực hiện thao tác này chứ ?</p>
                </div>
                <DialogFooter>
                  ...
                  <DialogClose as-child>
                    <Button variant="destructive" @click="$emit('deleteRequest', row.Ma_PR)">Xóa</Button>
                  </DialogClose>
                </DialogFooter>
              </DialogContent>
            </Dialog>
